...
def product(request, *args, **kwargs):
    ...
    some_data = serialize('json', page_obj.object_list)
    return JsonResponse(safe=False, data=some_data)
...
def products(request, *args, **kwargs):
    ...
    return render(request, 'cart/product_list.html', {'page_obj': page_obj})


def search_product_by_title(request, *args, **kwargs):
    title_product = request.GET.get('title')
    data = Product.objects.filter(title=title_product)
    serialized_data = serialize('json', data)
    return JsonResponse(safe=False, data=serialized_data)


def price_filter(request, *args, **kwargs):
    minimum = request.GET.get('MIN')
    maximum = request.GET.get('MAX')
    # minimum = kwargs['MIN']
    # maximum = kwargs['MAX']
    all_products = Product.objects.filter(Q(price__gt=minimum) | Q(price__lt=maximum))
    some_data = serialize('json', all_products)
    return JsonResponse(safe=False, data=some_data)


def product_search(request):
    query = request.GET.get('title')
    data = Product.objects.filter(title__icontains=query) if query else []

    return render(request, 'cart/search.html', {'products': data, 'query': query})
